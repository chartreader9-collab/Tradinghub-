...
def yf_prices(symbol: str, period="1y", interval="1d") -> pd.DataFrame:
    ...
    if "Date" not in df.columns and len(df.columns) > 0:
        # yfinance sometimes returns Datetime/Date in index
...
def parse_form4_buys(xml_text: str) -> list:
    ...
    def tag_endswith(e, suf):
        return e.tag.lower().endswith(suf)

    for ndt in root.iter():
        if tag_endswith(ndt, "nonderivativetable"):
            for txn in ndt:
                tcode = shares = price = None
                tdate = None
                for ch in txn.iter():
...
def prepare_cot_auto(cftc_df: pd.DataFrame, market_filter: str, price_symbol: str, window: int, pctl: float) -> pd.DataFrame:
    ...
    if sub.empty:
        raise ValueError("No rows found for that market filter.")
...
class TestTradingHubCore(unittest.TestCase):
    ...
    def test_prepare_cot_auto_with_dummy_prices(self):
        # Build small dummy CFTC frame (weekly Fridays)
        dates = pd.date_range("2023-01-06", periods=12, freq="W-FRI")
        cftc = pd.DataFrame({
            "Date": dates,
            "Market": ["GOLD - COMMODITY EXCHANGE INC."] * len(dates),
            "MM_LONG": np.linspace(200_000, 220_000, len(dates)),
            "MM_SHORT": np.linspace(210_000, 200_000, len(dates)),
        })
        # Monkeypatch yf_prices to avoid network
        def _fake_prices(symbol: str, period="10y", interval="1d"):
            # daily dates spanning the same period
            d = pd.date_range(dates.min() - pd.Timedelta(days=7), dates.max() + pd.Timedelta(days=7), freq="D")
            close = np.linspace(1800, 1900, len(d))
            df = pd.DataFrame({"Date": d, "Open": close, "High": close, "Low": close, "Close": close, "Volume": 0})
            return df
        globals()["yf_prices"] = _fake_prices

        merged = prepare_cot_auto(cftc, "GOLD", "GC=F", window=6, pctl=20)
        # Should have required columns and at least as many weekly rows as our price resample
        for col in ["MM_NET", "COT_Idx", "Washed", "TurnUp", "PriceUp", "Trigger"]:
...
if __name__ == "__main__":
    if not HAS_STREAMLIT:
        # Run tests when Streamlit UI isn't available
        ...
        # With Streamlit installed, the UI defined above will render when run via `streamlit run app.py`
        pass
